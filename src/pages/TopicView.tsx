...
const TopicView = () => {
  ...
  const [starredSubsections, setStarredSubsections] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    ...
    loadStarredSubsections();
  }, [id]);

  const loadStarredSubsections = async () => {
    try {
      ...
      const starredSet = new Set(data?.map(s => s.subsection_slug) || []);
      setStarredSubsections(starredSet);
    } catch (error) {
      console.error("Error loading starred subsections:", error);
    }
  };

  const handleToggleStar = async (subsectionId: string, subsectionTitle: string) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        toast.error("Please login to star subsections");
        return;
      }

      const isCurrentlyStarred = starredSubsections.has(subsectionId);
      ...
      if (isCurrentlyStarred) {
        ...
        setStarredSubsections(prev => new Set(prev).add(subsectionId));
        toast.success("Added to starred subsections!");
      }
    } catch (error) {
                ...
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <StarButton
                      ...
                      isStarred={starredSubsections.has(subsection.id)}
                      onToggle={() => handleToggleStar(subsection.id, subsection.title)}
                      size="sm"
                    />
                  </div>
                </CardHeader>
